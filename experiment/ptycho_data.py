...
class DataSet:
    def __init__(self, num_takes, title, directory, im_shape, pixel_size, distance, energy, is3d, verbose=False):
        ...
        self.translation = np.empty((self.N, 2))
        self.rotation = np.empty(self.N)
        self.is3d = is3d

        self.data = np.empty((self.N, self.im_shape[1], self.im_shape[0]), dtype='u2')
    ...
    def record_data(self, position, im_data):
        ...
        self.print(f'Take {self._n} of {self.N} recorded!')
        self._n += 1
        if self._n == self.N:
            self.finalize()
        return
...
class GeneratedData(DataSet):
    def __init__(self, ims_per_line, max_shift, title, directory, im_size=2 ** 7):
        super().__init__(ims_per_line**2, title, directory, (im_size, im_size), 5.5, 0.1, 1.4, True, True)
        self.data = None
        self.data = np.empty((self.N, self.im_shape[1], self.im_shape[0]))
        ...
        probe_radius = .003 / probe_pixel_size
        ...
        X, Y = np.meshgrid(x, x)
        X = np.ravel(X) * hf.random(self.N, 0.95, 1.05)
        Y = np.ravel(Y) * hf.random(self.N, 0.95, 1.05)
        ...
        for i in range(self.N):
            ...
            # plt.draw()
            # plt.pause(0.1)
            self.record_data(position*probe_pixel_size, diffraction)
...
if __name__ == '__main__':
    data = GeneratedData(8, 0.008, 'fake', 'C:/Users/jacio/OneDrive/Escritorio')
