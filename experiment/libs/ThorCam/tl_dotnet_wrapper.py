...
clr.AddReference('experiment/libs/ThorCam/Thorlabs.TSI.TLCamera')
clr.AddReference('experiment/libs/ThorCam/Thorlabs.TSI.TLCameraInterfaces')
clr.AddReference('experiment/libs/ThorCam/Thorlabs.TSI.ImageData')
clr.AddReference('experiment/libs/ThorCam/Thorlabs.TSI.ColorInterfaces')
...
class TL_Camera(object):
    ...
    def frame_to_array(frame) -> np.ndarray:
        ...
        try:
            ...
            destination = np.fromstring(ctypes.string_at(image_ptr, len(image)*2), dtype=np.ushort)  # 2 is the size of
                ushort in bytes
            ...
            Marshal.Copy(image, 0, IntPtr.__overloads__[Int64](ret_array.__array_interface__['data'][0]), len(image))
            # .NET method that copies image ( a System.UInt16[] type ) memory to ret_array ( an np array dtype = uint16
            # ) 'data' memory
